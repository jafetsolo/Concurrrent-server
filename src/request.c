...
// handle a request for a FIFO algorithm
...
// handle a request for an SFF algorithm
void request_handle_SFF(int fd, char *method, char *uri, char *version, struct stat sbuf)
{
    int is_static;
    char buf[MAXBUF];
    char filename[MAXBUF], cgiargs[MAXBUF];
